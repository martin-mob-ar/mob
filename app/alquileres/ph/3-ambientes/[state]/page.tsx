...
export async function generateStaticParams() {
  return buildStateParamsForFilter({ propertyTypeSlug: "ph", roomCount: ROOM_COUNTS["3-ambientes"].count });
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { state } = await params;
  const { data } = await supabaseAdmin.from("tokko_state").select("name").eq("slug", state).single();
  return buildProgrammaticMetadata({ propertyTypeSlug: "ph", roomSlug: "3-ambientes", stateSlug: state, stateName: data?.name || state });
}

export default async function Page({ params }: PageProps) {
  const { state } = await params;
  return <ProgrammaticSearchPage propertyTypeSlug="ph" roomSlug="3-ambientes" stateSlug={state} />;
}