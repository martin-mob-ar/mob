...
export async function generateStaticParams() {
  return buildStaticParamsForFilter({ propertyTypeSlug: "ph", roomCount: ROOM_COUNTS["3-ambientes"].count });
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildProgrammaticMetadata({ propertyTypeSlug: "ph", roomSlug: "3-ambientes", stateSlug: state, stateName: stateData?.name || state, locationSlug: location, locationName: locData?.name || location });
}

export default async function Page({ params }: PageProps) {
  const { state, location } = await params;
  return <ProgrammaticSearchPage propertyTypeSlug="ph" roomSlug="3-ambientes" stateSlug={state} locationSlug={location} />;
}
