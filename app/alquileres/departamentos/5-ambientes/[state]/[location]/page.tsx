...
export async function generateStaticParams() {
  return buildStaticParamsForFilter({ propertyTypeSlug: "departamentos", roomCount: ROOM_COUNTS["5-ambientes"].count });
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildProgrammaticMetadata({ propertyTypeSlug: "departamentos", roomSlug: "5-ambientes", stateSlug: state, stateName: stateData?.name || state, locationSlug: location, locationName: locData?.name || location });
}

export default async function Page({ params }: PageProps) {
  const { state, location } = await params;
  return <ProgrammaticSearchPage propertyTypeSlug="departamentos" roomSlug="5-ambientes" stateSlug={state} locationSlug={location} />;
}
