...
export async function generateStaticParams() {
  return buildStateParamsForFilter({ propertyTypeSlug: "departamentos", roomCount: ROOM_COUNTS["monoambiente"].count });
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { state } = await params;
  const { data } = await supabaseAdmin.from("tokko_state").select("name").eq("slug", state).single();
  return buildProgrammaticMetadata({ propertyTypeSlug: "departamentos", roomSlug: "monoambiente", stateSlug: state, stateName: data?.name || state });
}

export default async function Page({ params }: PageProps) {
  const { state } = await params;
  return <ProgrammaticSearchPage propertyTypeSlug="departamentos" roomSlug="monoambiente" stateSlug={state} />;
}