...
export async function generateStaticParams() {
  return buildStateParamsForFilter({ propertyTypeSlug: "casas", roomCount: ROOM_COUNTS["5-ambientes"].count });
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { state } = await params;
  const { data } = await supabaseAdmin.from("tokko_state").select("name").eq("slug", state).single();
  return buildProgrammaticMetadata({ propertyTypeSlug: "casas", roomSlug: "5-ambientes", stateSlug: state, stateName: data?.name || state });
}

export default async function Page({ params }: PageProps) {
  const { state } = await params;
  return <ProgrammaticSearchPage propertyTypeSlug="casas" roomSlug="5-ambientes" stateSlug={state} />;
}