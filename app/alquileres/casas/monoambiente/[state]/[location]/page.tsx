...
export async function generateStaticParams() {
  return buildStaticParamsForFilter({ propertyTypeSlug: "casas", roomCount: ROOM_COUNTS["monoambiente"].count });
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildProgrammaticMetadata({ propertyTypeSlug: "casas", roomSlug: "monoambiente", stateSlug: state, stateName: stateData?.name || state, locationSlug: location, locationName: locData?.name || location });
}

export default async function Page({ params }: PageProps) {
  const { state, location } = await params;
  return <ProgrammaticSearchPage propertyTypeSlug="casas" roomSlug="monoambiente" stateSlug={state} locationSlug={location} />;
}
