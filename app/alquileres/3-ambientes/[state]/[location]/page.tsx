...
export async function generateStaticParams() {
  return buildStaticParamsForFilter({ roomCount: ROOM_COUNTS["3-ambientes"].count });
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildProgrammaticMetadata({ roomSlug: "3-ambientes", stateSlug: state, stateName: stateData?.name || state, locationSlug: location, locationName: locData?.name || location });
}

export default async function Page({ params }: PageProps) {
  const { state, location } = await params;
  return <ProgrammaticSearchPage roomSlug="3-ambientes" stateSlug={state} locationSlug={location} />;
}
