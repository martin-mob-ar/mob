...
export async function generateStaticParams() {
  return buildStaticParamsForFilter({ roomCount: ROOM_COUNTS["4-ambientes"].count });
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildProgrammaticMetadata({ roomSlug: "4-ambientes", stateSlug: state, stateName: stateData?.name || state, locationSlug: location, locationName: locData?.name || location });
}

export default async function Page({ params }: PageProps) {
  const { state, location } = await params;
  return <ProgrammaticSearchPage roomSlug="4-ambientes" stateSlug={state} locationSlug={location} />;
}
