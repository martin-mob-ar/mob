-- Track price changes on properties for discount badges and chart annotations
-- ============================================================================

-- 1. New table: price_history
-- ============================================================================
...
CREATE TABLE public.price_history (
  ...
  CONSTRAINT price_history_operacion_fkey
...
-- Allow inserts from any context (trigger fires in caller's role)
CREATE POLICY "Allow trigger inserts" ON public.price_history
  FOR INSERT WITH CHECK (true);

-- Allow reads for rebuild_property_listing and API queries
...
AS $function$
BEGIN
  -- On UPDATE: record if price or currency changed
  IF TG_OP = 'UPDATE' THEN
    IF OLD.price IS DISTINCT FROM NEW.price
       OR OLD.currency IS DISTINCT FROM NEW.currency THEN
      INSERT INTO price_history (property_id, operacion_id, old_price, old_currency, new_price, new_currency)
      VALUES (NEW.property_id, NEW.id, OLD.price, OLD.currency, NEW.price, NEW.currency);
    END IF;
    RETURN NEW;
  END IF;

  -- On INSERT: record if this property already had a different price from its previous operation
  IF TG_OP = 'INSERT' THEN
    INSERT INTO price_history (property_id, operacion_id, old_price, old_currency, new_price, new_currency)
    SELECT NEW.property_id, NEW.id, prev.price, prev.currency, NEW.price, NEW.currency
    FROM operaciones prev
    WHERE prev.property_id = NEW.property_id
      AND prev.id != NEW.id
      AND (prev.price IS DISTINCT FROM NEW.price
           OR prev.currency IS DISTINCT FROM NEW.currency)
...
-- 3. Create trigger on operaciones
-- ============================================================================

-- Name chosen so it fires BEFORE trg_operaciones_sync_listing (alphabetical order)
CREATE TRIGGER trg_operacion_log_price
  AFTER INSERT OR UPDATE OF price, currency ON public.operaciones
  FOR EACH ROW EXECUTE FUNCTION public.track_price_change();

-- 4. Add columns to properties_read
-- ============================================================================

ALTER TABLE public.properties_read
...
-- 5. Update rebuild_property_listing() with price history columns
-- ============================================================================
...
BEGIN
  ...
  INSERT INTO properties_read (
    ...
    views_count,
    previous_price, previous_currency, price_changed_at, discount_pct
  )
  SELECT
    ...
    ph.changed_at,
    CASE
      WHEN ph.old_price IS NOT NULL
        AND ph.old_price > 0
        AND o.price IS NOT NULL
        AND o.price < ph.old_price
        AND ph.old_currency = o.currency
      THEN ROUND(((o.price - ph.old_price) / ph.old_price) * 100, 1)
      ELSE NULL
    END
  FROM properties p
  ...
  LEFT JOIN LATERAL (
    SELECT old_price, old_currency, changed_at
    FROM price_history
  ...
  GROUP BY p.id, p.tokko_id, pt.id, pt.name, l.id, l.name, l.depth, l.slug, pl.name, s.name, s.id, s.slug, c.name,
           ...
           ph.old_price, ph.old_currency, ph.changed_at
  ON CONFLICT (property_id) DO UPDATE SET
    ...
    previous_price=EXCLUDED.previous_price, previous_currency=EXCLUDED.previous_currency,
    price_changed_at=EXCLUDED.price_changed_at, discount_pct=EXCLUDED.discount_pct;
